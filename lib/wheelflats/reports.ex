defmodule Wheelflats.Reports do
  @moduledoc """
  The Reports context.
  """

  import Ecto.Query, warn: false
  alias Wheelflats.Repo
  ...
  def subscribe_reports(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(Wheelflats.PubSub, "user:#{key}:reports")
  end

  defp broadcast_report(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(Wheelflats.PubSub, "user:#{key}:reports", message)
  end
  ...
  def create_report(%Scope{} = scope, attrs) do
    with {:ok, report = %Report{}} <-
           ...
           |> Repo.insert() do
      broadcast_report(scope, {:created, report})
      {:ok, report}
    end
  end
  ...
  def update_report(%Scope{} = scope, %Report{} = report, attrs) do
    ...
    with {:ok, report = %Report{}} <-
           ...
           |> Report.changeset(attrs, scope)
           |> Repo.update() do
      broadcast_report(scope, {:updated, report})
      {:ok, report}
    end
  end
  ...
  def delete_report(%Scope{} = scope, %Report{} = report) do
    ...
    with {:ok, report = %Report{}} <-
      ...
      broadcast_report(scope, {:deleted, report})
      {:ok, report}
    end
  end
